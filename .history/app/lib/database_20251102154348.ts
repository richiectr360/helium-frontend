...
function ensureComponentsTable(): void {
  ...
  try {
    ...
    if (!exists) {
      ...
      // Check if chat_history column exists, add if not (migration for existing DBs)
      const tableInfo = db.exec("PRAGMA table_info(components)");
      const columns = tableInfo[0]?.values.map((row: SqlValue[]) => row[1]) || [];
      if (!columns.includes('chat_history')) {
        db.run("ALTER TABLE components ADD COLUMN chat_history TEXT DEFAULT ''");
        saveDatabaseToLocalStorage();
      }
    }
  } catch (e) {
...
export async function createComponent(
  ...
  db!.run(
    `INSERT INTO components (id, name, code, session_id, chat_history) VALUES (?, ?, ?, ?, ?)`,
    [rec.id, rec.name, rec.code, rec.session_id ?? "", rec.chat_history ?? ""]
  );
  saveDatabaseToLocalStorage();
}
...
export async function updateComponent(
  id: string,
  fields: Partial<Pick<ComponentRecord, "name" | "code" | "session_id" | "chat_history">>
): Promise<void> {
  if (!db) {
    await initializeDatabase();
  }
  ...
  if (fields.chat_history !== undefined) {
    sets.push("chat_history = ?");
    values.push(fields.chat_history);
  }
...
export async function getComponent(
  ...
  return {
    ...
    chat_history: (r[4] as string) || undefined,
...
export async function listComponents(): Promise<ComponentRecord[]> {
  ...
  return result[0].values.map((row) => ({
    ...
    chat_history: (row[4] as string) || undefined,
