...
export default function App() {
  ...
  // Show toast when component has finished rendering
  useEffect(() => {
    if (!isLoading && componentCode.trim() && !hasShownInitialToast && isMounted) {
      // Add a small delay to ensure Sandpack has fully rendered
  ...
  }, [isLoading, componentCode, hasShownInitialToast, isMounted, showToast]);

  // Reset toast flag when componentCode changes significantly
  useEffect(() => {
