...
export default function App() {
  return (
    ...
    const rewriteWithKeys = async () => {
      ...
      // Also replace object fields where the value is a key-like string (dynamic approach)
      // Match any prop: value pair where value is a key-like string
      const propKeySingle = /(\w+)\s*:\s*'(([A-Za-z]+)(?:\.[A-Za-z0-9_]+)+)'/g;
      const propKeyDouble = /(\w+)\s*:\s*"(([A-Za-z]+)(?:\.[A-Za-z0-9_]+)+)"/g;
      out = out.replace(propKeySingle, (m, propName, k) => {
        const resolvedKey = keyAliases[k] || k;
        return (allKeys.has(k) || allKeys.has(resolvedKey)) ? `${propName}: __i18n('${resolvedKey}')` : m;
      });
      out = out.replace(propKeyDouble, (m, propName, k) => {
        const resolvedKey = keyAliases[k] || k;
        return (allKeys.has(k) || allKeys.has(resolvedKey)) ? `${propName}: __i18n('${resolvedKey}')` : m;
      });
...
export default function App() {
  ...
  // Also increment key when processedCode changes to update translations
  useEffect(() => {
    if (processedCode) {
      setPreviewKey(k => k + 1);
    }
