...
export async function POST(req: Request) {
  try {
    const { translations }: { translations: Array<{ key: string, en: string }> } = await req.json();

    if (!translations || !Array.isArray(translations) || translations.length === 0) {
      return Response.json({ error: 'Invalid request: translations array required' }, { status: 400 });
    }
